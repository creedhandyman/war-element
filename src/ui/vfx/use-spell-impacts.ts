...
export function attackDeliveryMs(before: GameState, after: GameState): number {
  return effectsOn() && cardAttack(before, after) ? ATTACK_MS : 0;
}
    ...
    for (const f of fx) {
      switch (f.kind) {
        ...
        case "wall": {
          ...
          if (r) l.play({ kind: "wall", rect: r, element: f.element, variant: f.variant });
          break;
        }
        case "pulse": {
          const r = rowRect(f.row);
...
export function useSpellImpacts(game: GameState | null, inMatch: boolean, hold: boolean, viewer: PlayerId) {
  ...
  useEffect(() => {
    ...
    const fx = [
      ...spellEffects(before, game, viewer), ...trapsSprung(before, game), ...wallsCrossed(before, game),
      ...cardAttackEffects(before, game),
