...
export interface PlayerState {
  ...
  /** Midnight Shade (DUSK aura): one entry per fallen DUSK card whose shadow is
   *  still up — the last round it covers. Each is +5% dodge for this player's
   *  DUSK cards, capped at DUSK_SHADE_MAX_STACKS. A death writes `round + 1`, so
   *  its shadow covers the rest of the round the card fell in plus one whole
   *  round after — that is the "1 round" a player reads, since a death landing
   *  late in a battle queue would otherwise expire before anything could swing
   *  at the survivors. Each entry lifts ON ITS OWN in Cleanup once passed; a
   *  fresh death never extends an older one. Not a status — flat and
   *  uncleansed, like Misty's fog above. */
  shadeUntil?: number[];
