...
import { OPENING_COST_CAP, poolGainForRound } from "../types";
...
describe("opening deployment", () => {
  ...
  it("leaves the round economy exactly where it would have been", () => {
    ...
    //
    // The grant is the round's bracket plus one per home slot HELD, and a deployed
    ...
    for (const p of ["P1", "P2"] as PlayerId[])
      expect(s.players[p].gold, `${p} round-1 gold`)
        .toBeLessThanOrEqual(poolGainForRound(s.round) + homeSlotsHeld(s, p));
  });
