...
describe("medium-tier passives (audit batch)", () => {
  ...
  it("Ironclad's Magnetic Steel strips the rank ahead but damages the whole board", () => {
    ...
    // 5 printed + 3 magnetised off the near rank. Exostone used to add 2 more
    // here — one per plate the barrage broke — and no longer pays for breaks at
    // all, so the magnet's haul is the whole of the gain.
    expect(s.cards[steel.instanceId].curShields).toBe(8);
  });
  ...
  it("...and takes only what a foe actually has, never more", () => {
    ...
    // Only the 1 plate left for the magnet to take — "up to 3", not a flat 3.
    // (Exostone used to add another for the break itself; it no longer does.)
    expect(s.cards[steel.instanceId].curShields).toBe(1);
  });
