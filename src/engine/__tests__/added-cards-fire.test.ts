...
describe("wave 1: Crystal Sabor, Dynamo, Lumberjack, Bootlegger", () => {
  ...
  it("Lumberjack fells straight down its OWN column, 3 slots deep", () => {
    ...
    // All three slots in the lane, PEN straight through the 9 shields (Timberer 8).
    expect(60 - next.cards[near.instanceId].curHp).toBe(8);
    expect(60 - next.cards[mid.instanceId].curHp).toBe(8);
    expect(60 - next.cards[summonRow.instanceId].curHp).toBe(8); // landed in their home row
